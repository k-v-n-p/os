...
void producer ()		
{
  ...
  if (nextp == N)
    nextp = 0;
//Gain access to buffer and produce data for consumption from producer
...
void consumer ()
{
  ...
  if (nextc == N)
    nextc = 0;
  g = *(buffer + nextc++);
//Gain access to buffer and consume the available data avilable to consumer.
  printf ("\nConsumer consumes data %d", g);
}
//Signal operation on semaphores mutex and full using semop function.
